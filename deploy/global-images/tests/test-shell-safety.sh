...
grep -F 'gatewayApiKey: "${MEMORY_CORE_GATEWAY_API_KEY}"' "$ROOT/start-proxy.sh" >/dev/null
...
ruby -e 'require "yaml"; c=YAML.safe_load(File.read(ARGV[0])); abort unless c.dig("auth","gatewayApiKey")=="test-gateway-key" && c.dig("tdai","apiKey")=="test-gateway-key" && c.dig("skill","serviceToken")=="test-gateway-key"' "$TMP/config/config.yaml"
...
PATH="$TMP/bin:$PATH" "$TMP/deploy/start-proxy.sh" >/dev/null
ruby -e 'require "yaml"; c=YAML.safe_load(File.read(ARGV[0])); abort unless c.dig("auth","gatewayApiKey")=="" && c.dig("tdai","apiKey")=="" && c.dig("skill","serviceToken")==""' "$TMP/config/config.yaml"
